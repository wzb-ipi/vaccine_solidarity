...
mean(df_2$solidarity_behaviour,na.rm = TRUE)
median(df_2$solidarity_behaviour,na.rm = TRUE)
# most common 0.4 (3): 20 Mingle Points keep 45 Mingle Points donate
...
lm_robust(cash_billions ~ trading_factor + risk_factor*treatment_video,  se_type = "stata", data = df) %>% tidy  %>% 
  kbl(digit =2) %>%
  kable_minimal()
...
# LL  <- function(a=1,b=1,c=1,g=1,k=1, sigma=1, data) {
#     
#     R <- with(data, 
#           lik_x(x, 
#              sigma, 
...
# https://cran.r-project.org/web/packages/FENmlm/vignettes/FENmlm.html
# library(FENmlm)   
