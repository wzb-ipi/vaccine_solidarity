...
main_df <- read_csv('1_input/Merge_W1_W2_W3_W4_renamed.csv') %>% 
...
conjoints <- 
  ...
  list(
    conjoint1_1 =
      df_2 %>%
      mutate(vignr = as.numeric(exp3_vignette.1),
             outcome = exp3_choice.1, rating = exp3_rating.1, contest = 1, candidate=1) %>%
      select(ID, vignr, outcome, rating, contest, candidate),
    
  conjoint1_2 =
      df_2 %>%
      mutate(vignr = as.numeric(exp3_vignette.2),
             outcome = exp3_choice.1, rating = exp3_rating.2, contest = 1, candidate=2) %>%
      select(ID, vignr, outcome, rating, contest, candidate), 

    
  conjoint2_1 =
      df_2 %>%
      mutate(vignr = as.numeric(exp3_vignette.3),
             outcome = exp3_choice.2, rating = exp3_rating.3, contest = 2, candidate=1) %>%
      select(ID, vignr, outcome, rating, contest, candidate), 

        
    conjoint2_2 =
      df_2 %>%
      mutate(vignr = as.numeric(exp3_vignette.4),
             outcome = exp3_choice.2, rating = exp3_rating.4, contest = 2, candidate=2) %>%
      select(ID, vignr, outcome, rating, contest, candidate), 

            
                conjoint3_1 =
      df_2 %>%
      mutate(vignr = as.numeric(exp3_vignette.5),
             outcome = exp3_choice.3, rating = exp3_rating.5, contest = 3, candidate=1) %>%
      select(ID, vignr, outcome, rating, contest, candidate), 

                
                    conjoint3_2 =
      df_2 %>%
      mutate(vignr = as.numeric(exp3_vignette.6),
             outcome = exp3_choice.3, 
             rating = exp3_rating.6, contest = 3, candidate=2) %>%
      select(ID, vignr, outcome, rating, contest, candidate) 
